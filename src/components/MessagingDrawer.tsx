...
export const MessagingDrawer: React.FC = () => {
  ...
  const getCounterpart = (conversation: typeof activeConv) => {
    ...
    const currentIsSeller = String(conversation.sellerId) === String(currentUser.id);

    if (currentIsBuyer) {
      return {
        name: conversation.sellerName || conversation.shopName || 'Seller',
        role: 'Seller',
        context: conversation.shopName || '',
    ...
    if (currentIsSeller) {
      return {
        name: conversation.buyerName || 'Buyer',
        role: 'Buyer',
        context: conversation.shopName || '',
    ...
    return {
      name: conversation.sellerName || conversation.buyerName || conversation.shopName || 'User',
      role: 'User',
      context: conversation.shopName || '',
      avatar: conversation.shopAvatar || conversation.buyerAvatar || '',
    };
  };

  const getConversationLabel = (conversation: typeof activeConv) => {
    const counterpart = getCounterpart(conversation);
    const primary = counterpart.role === 'Seller'
      ? (conversation?.shopName || counterpart.name)
      : counterpart.name;
    return {
      primary: primary || 'Conversation',
      secondary: counterpart.role,
      title: [counterpart.name, counterpart.role, counterpart.context].filter(Boolean).join(' · '),
    };
  };
      ...
      <div className="w-full max-w-lg bg-white h-full shadow-2xl flex flex-col border-l border-gray-100 animate-in slide-in-from-right duration-300">
        <div className="px-4 py-4 sm:px-5 border-b border-gray-100 bg-white">
          <div className="flex items-center justify-between gap-3">
            {activeConv ? (
              <div className="flex items-center gap-3 min-w-0">
                <img
                  loading="lazy"
                  decoding="async"
                  src={counterpart.avatar || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100&auto=format&fit=crop&q=80'}
                  alt={counterpart.name}
                  className="w-10 h-10 rounded-xl object-cover ring-1 ring-gray-200 shrink-0"
                />
                <div className="min-w-0">
                  <h3 className="font-bold text-sm sm:text-base text-[#20243A] truncate">
                    {getConversationLabel(activeConv).primary}
                  </h3>
                  <p className="text-xs text-gray-500 truncate">
                    {counterpart.role}{counterpart.context && ` · ${counterpart.context}`}
                  </p>
                </div>
              </div>
            ) : (
              <div className="min-w-0">
                <h3 className="font-bold text-base text-[#20243A]">Messages</h3>
            ...
            <div className="flex items-center gap-1 shrink-0">
              {currentUser.role === 'seller' && (
                <button
                  onClick={() => setShowNewSellerChat(v => !v)}
                  className="p-2 rounded-full hover:bg-gray-100 text-[#8067E8] transition-colors"
                  title="New seller conversation"
                  aria-label="New seller conversation"
                >
          ...
          {currentUser.role === 'seller' && showNewSellerChat && (
            <div className="mt-3 p-3 bg-[#FAF8FE] border border-[#DDD4FF] rounded-2xl">
              ...
              <div className="max-h-44 overflow-y-auto space-y-1.5">
                {otherSellerShops.length === 0 ? (
                  <p className="text-xs text-gray-500 py-2">No other sellers are available yet.</p>
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-3 p-2 rounded-xl hover:bg-white text-left transition-colors"
                  >
                    <img loading="lazy" decoding="async" src={shop.avatar} alt={shop.name} className="w-9 h-9 rounded-xl object-cover" />
                    <span className="min-w-0 flex-1">
                      <span className="block text-xs font-bold text-[#20243A] truncate">{shop.name}</span>
                      <span className="block text-[10px] text-gray-500 truncate">Seller: {shop.ownerName || 'Seller'}</span>
                    </span>
                  </button>
                ))}
              </div>
        ...
        {conversations.length > 1 && (
          <div className="px-4 py-2.5 bg-[#F7F5F3] border-b border-gray-200/70 flex items-center gap-2 overflow-x-auto">
            {conversations.map(c => {
              ...
              return (
                <button
                  ...
                  title={label.title}
                  className={`px-3 py-2 rounded-xl text-left whitespace-nowrap transition-all border ${
                    c.id === activeConv?.id
                      ? 'bg-[#8067E8] text-white border-[#8067E8]'
                      : 'bg-white text-[#20243A] border-gray-200 hover:border-[#C9BEF7]'
                  }`}
                >
                  <span className="block text-xs font-bold">{label.primary}</span>
                  <span className={`block text-[10px] ${c.id === activeConv?.id ? 'text-white/80' : 'text-gray-500'}`}>
                    {label.secondary}{c.unreadCount > 0 && c.id !== activeConv?.id ? ` · ${c.unreadCount} new` : ''}
                  </span>
                </button>
              );
            })}
        ...
        {attachedProduct && (
          <div className="mx-4 mt-3 p-3 bg-[#FAF8FE] border border-[#DDD4FF] rounded-2xl flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <span className="text-[10px] uppercase font-bold text-[#8067E8]">Product</span>
        ...
        <div className="p-4 bg-white border-t border-gray-100">
          <form onSubmit={e => { e.preventDefault(); handleSend(); }} className="flex items-center gap-2">
            <input value={inputMessage} onChange={e => setInputMessage(e.target.value)} placeholder={activeConv ? `Message ${counterpart.name || 'user'}...` : 'Select a conversation...'} disabled={!activeConv} className="flex-1 px-4 py-3 bg-[#F7F5F3] rounded-full text-sm font-medium text-[#20243A] placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#8067E8]/40 border border-gray-200/80 disabled:opacity-60" />
            <button type="submit" disabled={!activeConv || !inputMessage.trim()} className="p-3 bg-[#8067E8] hover:bg-[#6E52E2] disabled:opacity-50 text-white rounded-full shadow-md" aria-label="Send message"><Send className="w-4 h-4" /></button>
          </form>
        </div>
      </div>
    </div>
