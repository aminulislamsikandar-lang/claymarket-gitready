...
export const AuthModal: React.FC = () => {
  const { isAuthModalOpen, setIsAuthModalOpen, authModalTab, setAuthModalTab, loginUser, registerAccount, registerSeller, markets, categories, currentUser } = useApp();
  ...
  const handleModalClose = () => { if (!loading) setIsAuthModalOpen(false); };
  const modalRef = useFocusTrap(isAuthModalOpen, handleModalClose);

  useEffect(() => {
    if (isAuthModalOpen && authModalTab === 'become-seller' && currentUser.role !== 'guest') {
      setOwnerName(currentUser.name || ''); setPhone(currentUser.phone || ''); setMarketName(currentUser.sellerLocation?.marketName || ''); setSelectedMarketId(currentUser.sellerLocation?.marketId || 'mkt_kachumara');
    }
    if (!isAuthModalOpen) { setLoading(false); setError(''); setPassword(''); setPasswordVisible(false); setMarketSuggestionsOpen(false); }
  }, [isAuthModalOpen, authModalTab, currentUser]);

  useEffect(() => {
    const query = marketName.trim();
    if (query.length < 3) { setMarketSuggestions([]); setMarketLoading(false); return; }
    const requestId = ++marketRequestRef.current; setMarketLoading(true);
    const timer = window.setTimeout(async () => {
      try { const results = await searchPlaces(query); if (marketRequestRef.current === requestId) setMarketSuggestions(results); }
      catch { if (marketRequestRef.current === requestId) setMarketSuggestions([]); }
      finally { if (marketRequestRef.current === requestId) setMarketLoading(false); }
    }, 500);
    return () => window.clearTimeout(timer);
  }, [marketName]);

  useEffect(() => {
    const outside = (event: MouseEvent) => { if (marketRef.current && !marketRef.current.contains(event.target as Node)) setMarketSuggestionsOpen(false); };
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => { e.preventDefault(); resetStatus(); setLoading(true); try { await loginUser(email.trim(), password); } catch (err) { setError(err instanceof Error ? err.message : 'Unable to sign in. Please try again.'); } finally { setLoading(false); } };
  const handleRegisterSubmit = async (e: React.FormEvent) => { e.preventDefault(); resetStatus(); setLoading(true); try { await registerAccount({ name: fullName.trim(), email: email.trim(), password, phone: phone.trim() }); } catch (err) { setError(err instanceof Error ? err.message : 'Unable to create your account. Please try again.'); } finally { setLoading(false); } };
  ...
  const saveCustomMarketSeller = async () => {
    ...
    const cleanName = ownerName.trim(), cleanShop = shopName.trim(), cleanMarket = marketName.trim(), cleanState = stateName.trim(), cleanDistrict = districtName.trim(), cleanAddress = address.trim();
    const category = categories.find(c => c.id === selectedCategoryId);
    if (!cleanName) throw new Error('Please enter your name.'); if (!cleanShop) throw new Error('Please enter a shop name.'); if (!cleanMarket) throw new Error('Please enter or select a market.'); if (!cleanState) throw new Error('Please enter your state.'); if (!cleanDistrict) throw new Error('Please enter your district.'); if (!category) throw new Error('Please select a valid category.');
    const slug = cleanMarket.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'local';
    const customMarketId = `custom_market_${slug}`; const shopId = `shop_${firebaseUser.uid}`; const shopSlug = cleanShop.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'local-shop';
    if (firebaseUser.displayName !== cleanName) await updateProfile(firebaseUser, { displayName: cleanName });
    const shopDoc: Record<string, unknown> = { ownerId: firebaseUser.uid, ownerName: cleanName, name: cleanShop, slug: `${shopSlug}-${Date.now()}`, marketId: customMarketId, marketName: cleanMarket, state: cleanState, district: cleanDistrict, categoryIds: [category.id], categoryId: category.id, categoryName: category.name, profileImage: '', coverImage: '', description: '', phone: phone.trim(), address: cleanAddress, rating: 0, reviewsCount: 0, verified: false, followersCount: 0, createdAt: serverTimestamp(), updatedAt: serverTimestamp() };
    if (selectedMarketPlace) { shopDoc.marketLatitude = selectedMarketPlace.lat; shopDoc.marketLongitude = selectedMarketPlace.lon; shopDoc.marketPlaceLabel = selectedMarketPlace.label; shopDoc.marketPlaceSublabel = selectedMarketPlace.sublabel; }
    const batch = writeBatch(firebaseDb);
    batch.set(doc(firebaseDb, 'users', firebaseUser.uid), { name: cleanName, email: firebaseUser.email || '', phone: phone.trim(), role: 'seller', avatar: firebaseUser.photoURL || '', addresses: currentUser.addresses || [], shopId, shopName: cleanShop, sellerLocation: { state: cleanState, district: cleanDistrict, marketId: customMarketId, marketName: cleanMarket }, updatedAt: serverTimestamp() }, { merge: true });
    batch.set(doc(firebaseDb, 'shops', shopId), shopDoc); await batch.commit(); await firebaseUser.getIdToken(true); window.location.reload();
  };
  ...
  const handleBecomeSellerSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try { const existingMarket = markets.find(m => m.id === selectedMarketId && m.name === marketName.trim()); if (existingMarket) await registerSeller({ name: ownerName.trim(), phone: phone.trim(), shop: { name: shopName.trim(), marketId: existingMarket.id, categoryId: selectedCategoryId, state: stateName.trim(), district: districtName.trim(), address: address.trim() } }); else await saveCustomMarketSeller(); }
    catch (err) { setError(err instanceof Error ? err.message : 'Unable to create your seller account. Please try again.'); }
    finally { setLoading(false); }
  };

  const switchTab = (tab: 'login' | 'register' | 'become-seller') => { resetStatus(); setPassword(''); setPasswordVisible(false); setMarketSuggestionsOpen(false); setAuthModalTab(tab); };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200" role="presentation">
      <div ref={modalRef} role="dialog" aria-modal="true" aria-labelledby="claymarket-auth-title" className="bg-white rounded-3xl max-w-md w-full max-h-[calc(100vh-2rem)] overflow-y-auto p-6 sm:p-8 shadow-2xl border border-white/90 relative animate-in zoom-in-95 duration-200" style={{ boxShadow: '0 24px 48px -12px rgba(32, 36, 58, 0.2), inset 0 2px 4px rgba(255, 255, 255, 0.95)' }}>
        <button onClick={handleModalClose} aria-label="Close authentication dialog" title="Close" className="absolute top-5 right-5 p-2 rounded-full hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors"><X className="w-5 h-5" /></button>
        <div className="flex items-center justify-center gap-2 mb-6 p-1 bg-[#F7F5F3] rounded-2xl">{(['login', 'register', 'become-seller'] as const).map(tab => <button key={tab} onClick={() => switchTab(tab)} className={`flex-1 py-2 px-3 rounded-xl font-bold text-xs transition-all ${authModalTab === tab ? tab === 'become-seller' ? 'bg-[#8067E8] text-white shadow-xs' : 'bg-white text-[#20243A] shadow-xs' : tab === 'become-seller' ? 'text-[#8067E8] hover:bg-[#DDD4FF]/50' : 'text-[#737B89] hover:text-[#20243A]'}`}>{tab === 'login' ? 'Sign In' : tab === 'register' ? 'Create Account' : 'Become Seller'}</button>)}</div>
        {authModalTab !== 'become-seller' ? (
          <div className="space-y-4"><div className="text-center space-y-1"><h3 id="claymarket-auth-title" className="text-2xl font-extrabold text-[#20243A]">{authModalTab === 'login' ? 'Welcome Back!' : 'Create an Account'}</h3><p className="text-xs text-[#737B89]">{authModalTab === 'login' ? 'Access your orders, wishlist and direct seller chat.' : 'Create your real Claymarket account.'}</p></div>
