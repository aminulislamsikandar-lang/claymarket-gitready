...
export const CartDrawer: React.FC = () => {
  const { 
    isCartOpen, setIsCartOpen, cart, updateCartQuantity, 
    removeFromCart, clearCart, createOrder, navigateTo, showToast 
  } = useApp();
  ...
  const subtotal = cart.reduce((sum, item) => {
    const itemPrice = item.product.price !== undefined && item.product.price !== null ? item.product.price : 0;
        ...
        <div className="flex-1 p-5 overflow-y-auto space-y-3.5 bg-[#FDFCFB]">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div
                key={item.id}
                className="p-3.5 bg-white rounded-2xl border border-gray-100 shadow-xs flex items-center gap-3.5"
              >
                <img loading="lazy" decoding="async"
                  src={item.product.images[0]}
                  alt={item.product.name}
                  className="w-16 h-16 rounded-xl object-cover ring-1 ring-gray-100 shrink-0"
                />
                
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-xs sm:text-sm text-[#20243A] truncate">
                    {item.product.name}
                  </h4>
                  <p className="text-[11px] text-[#8067E8] font-semibold truncate">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <span className="font-extrabold text-sm text-[#20243A]">
          ...
          <div className="p-5 bg-white border-t border-gray-100 space-y-4">
            ...
            <div className="space-y-1.5 text-xs text-[#737B89]">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-[#20243A]">₹{subtotal}</span>
              </div>
              <div className="flex justify-between">
                <span>Fulfillment Fee</span>
                <span className="font-bold text-[#20243A]">{deliveryFee === 0 ? 'FREE' : `₹${deliveryFee}`}</span>
              </div>
              <div className="flex justify-between text-sm font-extrabold text-[#20243A] pt-2 border-t border-gray-100">
                <span>Total Amount</span>
                <span className="text-base text-[#8067E8]">₹{total}</span>
              </div>
            </div>
