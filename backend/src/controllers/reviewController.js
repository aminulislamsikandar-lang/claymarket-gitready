...
export async function createReview(req, res) {
  ...
  // Deterministic ID makes review identity stable across retries. createIfAbsent
  // uses Firestore's atomic create operation, so concurrent requests cannot
  // overwrite one another after both pass the existence check.
  ...
  try {
    const review = await Review.createIfAbsent({
      _id: reviewId,
    ...
    // Firestore ALREADY_EXISTS means another concurrent request created the
    // same deterministic review first.
    if (error?.code === 6 || error?.code === '6' || error?.code === 'already-exists') {
      return fail(res, 'You have already reviewed this item for this order.', 409);
    }
    throw error;
  }
}
